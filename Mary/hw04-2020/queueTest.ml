...
(* Now, it's your turn! Make sure to comprehensively test all the
   ...
   finish writingb tests for `truncate` and `delete`.
...
(*enqueing a char in a int queue (or other data type) *)
...
(* dequeing something not in list *)
let test () : bool =
  let q = create () in
  enq 2 q;
  enq 3 q;
  let hd = deq q in
  hd = 1 && to_list q = [3]
;; run_failing_test "deq on item not in list" test
...
let test () : bool =
  ...
  let hd = deq q in
  hd = 2 && to_list q = [3]
;; run_test "to_list enq and deq" test
...
let test () : bool =
  ...
  let hd = deq q in
  hd = 2 && length q = 1
;; run_test "length enq and deq non empty" test

(* deques on empty queue *)
let test () : bool =
  let q = create () in
  deq q;
  length q = 0
;; run_failing_test "length deq on empty" test
...
let test () : bool =
  let q = from_list [1; 2; 3] in
  enq 4 q;
  valid q && to_list q = [1; 2; 3; 4]
;; run_failing_test "from_list enq" test

(* deq *)
let test () : bool =
  let q = from_list [1; 2; 3] in
  let hd = deq q in
  hd = 1 && valid q && to_list q = [2; 3]
;; run_failing_test "from_list enq" test
...
let test () : bool =
  ...
  let hd = deq q in
  hd = 2 && contains 3 q
;; run_test "contains deq returns true" test

(* deq returns false*)
let test () : bool =
  let q = from_list [2; 3] in
  let hd = deq q in
  hd = 2 && not (contains 2 q)
;; run_test "contains deq returns false" test
...
let test () : bool =
  ...
  enq 4 q;
  contains 4 q
;; run_test "contains enq returns true" test
...
let test () : bool =
  ...
  enq 4 q;
  not (contains 5 q)
;; run_test "contains enq returns false" test
...
let test () : bool =
  ...
  truncate 6 q;
  is_empty q
;; run_test "truncate on empty queue " test
...
let test () : bool =
  ...
  truncate 4 q;
  to_list q = [4]
;; run_test "truncate on singleton queue elt in queue" test
...
let test () : bool =
  ...
  truncate 5 q;
  to_list q = [4]
;; run_test "truncate on singleton queue elt not in queue " test
...
let test () : bool =
  ...
  truncate 1 q;
  to_list q = [1]
;; run_test "truncate elt at head" test
...
let test () : bool =
  ...
  truncate 4 q;
  to_list q = [1; 2; 3; 4]
;; run_test "truncate on elt at tail" test
...
let test () : bool =
  ...
  truncate 2 q;
  to_list q = [1; 2]
;; run_test "truncate on elt in middle" test
...
let test () : bool =
  ...
  truncate 5 q;
  to_list q = [1; 2; 3; 4]
;; run_test "truncate elt not in queue" test
...
let test () : bool =
  ...
  delete 6 q;
  is_empty q
;; run_test "delete from empty queue " test
...
let test () : bool =
  ...
  enq 4 q;
  delete 4 q;
  to_list q = [] && is_empty q
;; run_test "delete single elt in queue" test

(* deletes single elt in non empty queue *)
let test () : bool =
  let q = from_list [3] in
  enq 4 q;
  delete 3 q;
  to_list q = [4] && not(is_empty q) 
;; run_test "delete single elt in non empty queue" test

(* multiple values for deletion *)
let test () : bool =
  let q = from_list [3; 3; 3; 3] in
  delete 3 q;
  to_list q = [] && is_empty q
;; run_test "delete all instances of elt in queue only containing elt" test

(* value for deletion not in list *)
let test () : bool =
  let q = from_list [1; 2; 3; 3] in
   delete 4 q;
  to_list q = [1; 2; 3; 3]  && not(is_empty q)
;; run_test "delete elt not in queue" test
