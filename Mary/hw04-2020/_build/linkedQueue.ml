...
module LinkedQueue : QueueInterface.QUEUE = struct
  ...
  let from_list (l: 'a list) : 'a queue =
    fold (fun x acc -> if is_empty acc 
    then (enq x acc ; acc) else let newnode = 
     {v = x ; next = acc.head} in 
     acc.head <- Some newnode ; acc ) (create()) l
  ...
  let contains (elt: 'a) (q: 'a queue) : bool =
    ...
    let hd = q.head in
    let rec infunc (e: 'a) (n: 'a qnode option) : bool = 
        begin match n with
        | None -> false
        | Some x -> if x.v == elt then true else infunc e x.next
        end
    in infunc elt hd
  ...
  *)
  ...
  let truncate (elt: 'a) (q: 'a queue) : unit =
    ...
    let hd = q.head in
    let rec infunc (e: 'a) (n: 'a qnode option) : unit = 
        begin match n with
        | None -> ()
        | Some x -> if x.v == elt then (x.next <- None ; q.tail <- Some x) 
                    else infunc e x.next
        end
    in infunc elt hd
  ...
  let delete (elt: 'a) (q: 'a queue) : unit =
    ...
    let hd = q.head in
    let rec infunc (e: 'a) (n: 'a qnode option) (pren: 'a qnode option) : unit = 
        begin match n with
        | None -> ()
        | Some x -> if x.v = elt then match pren with 
            | None -> q.head <- x.next ; infunc e q.head None
            | Some y -> y.next <- x.next  ; infunc e y.next (Some y)
                    else infunc e x.next (Some x)
        end
    in infunc elt hd None
