...
let deque_12 () : int deque =
  let n = { v = 1; prev = None; next = None } in
  let n2 = { v = 2 ; prev = Some n ; next = None} in
  n.next <- Some n2 ;
  { head = Some n; tail = Some n2 }
...
let to_list (q: 'a deque) : 'a list =
  ...
  let rec infunc (qn: 'a dqnode option) (l: 'a list) : 'a list =
      begin match qn with
      | None -> l
      | Some x -> infunc x.prev (x.v::l)
      end
   in infunc q.tail []
...
let insert_head (x: 'a) (q: 'a deque) : unit =
  ...
  let newnode = { v = x ; next = q.head; prev = None} in
  begin match q.head with
  | None -> q.tail <- Some newnode
  | Some x -> x.prev <- Some newnode
  end;
  q.head <- Some newnode

let insert_tail (x: 'a) (q: 'a deque) : unit =
  if not (valid q) then failwith "insert_tail: given invalid deque";
  let newnode = { v = x ; next = None; prev = q.tail} in
  begin match q.tail with
  | None -> q.head <- Some newnode
  | Some x -> x.next <- Some newnode
  end;
  q.tail <- Some newnode
...
let remove_head (q: 'a deque) : 'a =
  ...
  begin match q.head with
  | None -> failwith "remove_head: given invalid deque"
  | Some x -> begin match x.next with
      | None -> q.head <- None ; q.tail <- None ; x.v
      | Some y -> y.prev <- None ; q.head <- Some y ; x.v
      end
  end
...
let remove_tail (q: 'a deque) : 'a =
  if not (valid q) then failwith "remove_tail: given invalid deque";
   begin match q.tail with
  | None -> failwith "remove_tail: given invalid deque"
  | Some x -> begin match x.prev with
      | None -> q.head <- None ; q.tail <- None ; x.v
      | Some y -> y.next <- None ; q.tail <- Some y ; x.v
      end
  end
...
let delete_last (v: 'a) (q: 'a deque) : unit =
  ...
  let rec infunc (dqn: 'a dqnode option) : unit =
  begin match dqn with
  | None -> ()
  | Some x -> if x.v = v then (begin match x.next with
      | None -> begin match x.prev with
          | None -> q.head <- None ; q.tail <- None
          | Some z -> z.next <- None ; q.tail <- Some z
          end
      | Some y -> begin match x.prev with
          | None -> y.prev <- None ; q.head <- Some y
          | Some z -> z.next <- Some y ; y.prev <- Some z
          end
      end
      )
      else (infunc x.prev)
...
let reverse (q: 'a deque) : unit =
  ...
  let rec infunc (dqno: 'a dqnode option) (num: int) : unit =
    match dqno with
    | None -> ()
    | Some x -> let temp = x.next in
                x.next <- x.prev;
                x.prev <- temp;
