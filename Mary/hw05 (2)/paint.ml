...
let repaint (g: gctx) : unit =
  let draw_shape (s: shape) : unit =
    begin match s with
      ...
      | Points pl -> draw_points (with_params g pl.color) 
      pl.points
      | Ellipse e -> let e1 = (fst e.p1 + fst e.p2) / 2 in 
                      let e2 = (snd e.p1 + snd e.p2) / 2 in 
      draw_ellipse (with_params g e.color ) (e1, e2)
  ...
  begin match (event_type event) with
    | MouseDown ->
      ...
      (begin match paint.mode with 
         ...
         | EllipseEndMode p1 ->  
         Deque.insert_tail (Ellipse {color=paint.color; p1=p1; p2=p}) paint.shapes;
...
let mode_toolbar : widget = hlist [w_undo; w_line; w_point; w_ellipse]  
...
let paint_widget =
   vlist [mode_toolbar ; spacer; color_toolbar;  paint_canvas] 
