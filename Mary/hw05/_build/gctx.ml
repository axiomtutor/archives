...
let with_line_width (g: gctx) (w: int) : gctx =
  { g with line_width = w }
...
let set_graphics_state (gc: gctx) : unit =
  ...
  Graphics.set_line_width gc.line_width
...
(* takes in a graphics context and the coordinates of a point and
      draws a point in that location using the context *)
let draw_point (g: gctx ) (p: position) : unit =
 ...
 (* iterates over a list of points and draws each of them using the graphics context.*)
 let rec draw_points (g: gctx) (pl: position list) : unit =
   begin match pl with
   | [] -> ()
   | x::xs -> draw_point g x ; draw_points g xs
  end
