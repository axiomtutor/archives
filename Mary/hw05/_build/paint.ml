...
type shape =
  ...
  | Points of { color: Gctx.color; points: point list; line_width: int }
  | Ellipse of {color: color;  p1: point; p2: point; line_width: int }
...
let with_params (g: gctx) (c: color) (w: int) : gctx =
  let g = with_line_width (with_color g c) w in
  g
...
let repaint (g: gctx) : unit =
  let draw_shape (s: shape) : unit =
    begin match s with
      | Line l -> draw_line (with_params g l.color l.line_width) l.p1 l.p2
      | Points pl -> draw_points (with_params g pl.color pl.line_width)
      pl.points
      | Ellipse e -> let e1 = (fst e.p1 + fst e.p2) / 2 in
                      let e2 = (snd e.p1 + snd e.p2) / 2 in
      draw_ellipse (with_params g e.color e.line_width) (e1, e2)
      (abs (fst e.p1 - e1 )) (abs(snd e.p2 - e2))
    end in
  ...
  begin match (event_type event) with
    | MouseDown ->
      ...
      (begin match paint.mode with
         ...
         Deque.insert_tail (Line {color=paint.color; p1=p1; p2=p; line_width = paint.line_width}) paint.shapes;
                      paint.mode <- LineStartMode ;
                      paint.preview <- None
         | PointMode -> paint.preview <-
                        Some (Points {color=paint.color; points = [p]; line_width = paint.line_width})

         | EllipseStartMode ->  paint.mode <- EllipseEndMode p
         | EllipseEndMode p1 ->
         Deque.insert_tail (Ellipse {color=paint.color; p1=p1; p2=p;
                                     line_width = paint.line_width}) paint.shapes;
    ...
    | MouseDrag ->
      (* In this case, the mouse has been clicked, and it's being dragged
         ...
         | LineStartMode -> ()
         | LineEndMode p1 ->
          paint.preview <- Some (Line {color = paint.color; p1 = p1 ; p2 = p; line_width = paint.line_width})
         | PointMode -> let points =
              begin match paint.preview with
              | Some (Points ps) -> ps.points
              | _ -> []
              end in paint.preview <- Some (Points {color = paint.color;
                                            points = p::points; line_width = paint.line_width})
         | EllipseStartMode ->  ()
         | EllipseEndMode p1 ->
          paint.preview <- Some (Ellipse {color = paint.color; p1 = p1 ; p2 = p; line_width = paint.line_width})
      ...
      (* 3, 4, and possibly 6 need to do something different here.           *)
       begin match paint.mode with
         ...
         | PointMode -> begin match paint.preview with
           | Some x -> Deque.insert_tail x paint.shapes ; paint.preview <- None
           | None -> ()
           end

         | EllipseStartMode -> ()
         | EllipseEndMode p1 ->
         Deque.insert_tail (Ellipse {color=paint.color; p1=p1; p2=p; line_width = paint.line_width}) paint.shapes;
                      paint.mode <- EllipseStartMode ;
                      paint.preview <- None
          end
...
let thick (b : bool) : unit =
  if b then paint.line_width <- 10 else paint.line_width <- 1

;; vc_thick.add_change_listener thick

let (w_rs, vc_rs) = slider 0 "red slider" Gctx.red

let red_slider (x: int) : unit =
  let {r=r; g=g; b=b} = paint.color in
  print_int r; print_string ", "; print_int g; print_string ", "; print_int b;
  print_endline "";
  let r' = int_of_float (256.0 *. (float_of_int x /. 100.0)) in 
  paint.color <- { r=r'; g=g; b=b}

;; vc_rs.add_change_listener red_slider
...
let mode_toolbar : widget = hlist [w_undo; w_line; w_point; w_ellipse; w_thick]
...
let colored_square (width:int) (get_color:unit -> color)
  ...
  let repaint_square (gc:gctx) =
    let c = get_color () in
    fill_rect (with_params gc c 1) (0, 0) (width-1, width-1) in
...
let paint_widget =
   vlist [mode_toolbar ; spacer; color_toolbar; spacer; w_rs; paint_canvas]
