...
let hpair (w1:widget) (w2:widget) : widget = {
   repaint = (fun  (g:Gctx.gctx) -> w1.repaint g;
    let g = Gctx.translate g (fst (w1.size ()),0) in
      w2.repaint g);
   handle = (fun (g:Gctx.gctx) (e:Gctx.event) ->
    if event_within g e (w1.size ())    (* g graphics context, e is event*)
    then w1.handle g e
...
let make_controller (v: 'a) : 'a value_controller =
  ...
  {
    add_change_listener = (fun (cl: 'a -> unit) : unit ->
      listeners.contents <- cl::listeners.contents);
    get_value = (fun () : 'a -> value.contents);
    change_value = (fun (x : 'a) : unit -> value.contents <- x;
      List.iter (fun f -> f x) listeners.contents)
  }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  let bvc = make_controller init in
  let w = {
    repaint = (fun (g: Gctx.gctx) : unit -> (Gctx.draw_string g (0,0) s;
               if (bvc.get_value ()) then let (x,_) = Gctx.text_size s in
                                          Gctx.draw_string g (x,0) "X"));
    handle = (fun (g: Gctx.gctx) (e: Gctx.event) ->
              if Gctx.event_type e = Gctx.MouseDown
              then bvc.change_value (not (bvc.get_value ())));
    size = (fun () -> let (x,y) = Gctx.text_size s in
                      (x, y+10))
  } in
  (w, bvc)
...
let slider (init: int) (s: string) (c: Gctx.color) :
                                          widget * int value_controller =
  let ivc = make_controller init in
  let w = {
    repaint = (fun (g: Gctx.gctx) : unit ->
      let g' = Gctx.with_color g Gctx.white in
      let g'' = Gctx.with_line_width g' 1 in
      Gctx.fill_rect g'' (0, 0) (100, 15);
      Gctx.fill_rect (Gctx.with_color g'' c) (0, 0) ((ivc.get_value ()), 15);
      Gctx.draw_string (Gctx.with_color g'' Gctx.black) (100, 0) s);
    handle = (fun (g: Gctx.gctx) (e: Gctx.event) ->
      if Gctx.event_type e = MouseDown
        then (ivc.change_value (fst (Gctx.event_pos e g))));
    size = (fun () -> let (x,y) = Gctx.text_size s in
                      (x+100, max y 15))
  } in
  (w, ivc)
