...
module MakeInterval (Endpoint : ORDERED_TYPE) = 
  struct
    ...
    let contains (intvl : interval) (x : Endpoint.t) : bool =
      match intvl with 
      | Empty -> false
      | Interval (low, high) -> (compare x low >= 0) && (compare high x >= 0)

    (* intersect intvl1 intvl2 -- Returns the intersection of `intvl1`
       and `intvl2` *)
    let end_max (e1 : Endpoint.t) (e2 : Endpoint.t) : Endpoint.t = 
      if compare e1 e2 > 0 then e1 else e2 
    let end_min (e1 : Endpoint.t) (e2 : Endpoint.t) : Endpoint.t = 
      if end_max e1 e2 = e1 then e2 else e1

    let intersect (intvl1 : interval) (intvl2 : interval) : interval =
      match intvl1, intvl2 with 
      | Interval (low1, high1), Interval (low2, high2) -> 
          create (end_max low1 low2) (end_min high1 high2)
      | _ -> Empty
    end ;;
