...
module ListQueue (Elt : COMPARABLE)
  ...
  struct
    ...
    let add (e : elt) (q : queue) : queue = e::q
    ...
    let one_test () = 
      ...
      let update1 = add x empty in
      let update2 = take update1 in 
      assert (update1 = [x]);
      assert (update2 = (x,[]))
    let many_test () = 
      let x = Elt.generate () in
      let x2 = Elt.generate_lt x in
      let x3 = Elt.generate_lt x in 
      let step1 = add x empty in
      let step2 = add x2 step1 in
      let step3 = add x3 step2 in 
      let step4 = add x step3 in 
      let (out, rem) = take step4 in 
      assert (out = x3);
      assert (rem = step3)
...
module TreeQueue (Elt : COMPARABLE) : (PRIOQUEUE with type elt = Elt.t) =
  struct
    ...
    let take (q : queue) : elt * queue =
      let minimum = T.getmin q in (minimum, T.delete minimum q) 
...
module BinaryHeap (Elt : COMPARABLE) : (PRIOQUEUE with type elt = Elt.t) =
  struct
    ...
    *)
    ...
    ..................................................................*)
    let reassignRoot (e : elt) (t : tree) : tree = 
      match t with 
      | Leaf _ -> Leaf e 
      | OneBranch (_, d2) -> OneBranch (e, d2)
      | TwoBranch (parity, d1, t1, t2) -> TwoBranch (parity, e, t1, t2) 
    
    let rec fix (t : tree) : tree =
      match t with 
      | Leaf _ -> t
      | OneBranch (dO1, dO2) -> (match Elt.compare dO1 dO2 with 
                                 | Greater -> OneBranch (dO2, dO1)
                                 | _ -> t)
      | TwoBranch (parity, d, tLeft, tRight) -> let leftValue = get_top tLeft in 
                                                let rightValue = get_top tRight in 
                                                (match Elt.compare d leftValue, Elt.compare d rightValue, Elt.compare leftValue rightValue with 
                                                | Less, Greater, _
                                                | Greater, Greater, Greater -> TwoBranch (parity, rightValue, tLeft, fix (reassignRoot d tRight))
                                                | Greater, Less, _ 
                                                | Greater, Greater, _ -> TwoBranch (parity, leftValue, fix (reassignRoot d tLeft), tRight)
                                                | _ -> t )
    ...
    let rec get_last (t : tree) : elt * queue =
      match t with 
      | Leaf d -> (d, Empty) 
      | OneBranch (dTop, dBottom) -> (dBottom, Tree (Leaf dTop))
      | TwoBranch (Even, d, tLeft, tRight) -> let (e, newQueue) = get_last tRight in 
                                              (match newQueue with 
                                               | Empty -> (e, Tree (OneBranch (d, get_top tLeft)))
                                               | Tree newTree -> (e, Tree (TwoBranch (Odd, d, tLeft, newTree))))
      | TwoBranch (Odd, d, tLeft, tRight) -> let (e, newQueue) = get_last tLeft in 
                                             (match newQueue with
                                              | Empty -> failwith "This line shouldn't be reachable"
                                              | Tree newTree -> (e, Tree (TwoBranch (Even, d, newTree, tRight)))) 
    ...
    let take (q : queue) : elt * queue =
      ...
      | TwoBranch (Odd, _e, _t1, _t2) -> 
         let (last, q2') = get_last _t1 in
         (match q2' with 
          (* If one branch of the tree was ... *)
          | Empty -> failwith "This line shouldn't be reachable"
...
let example_list = [5;4;2;5;1;2] in 
let result = [1;2;2;4;5;5] in
assert (selectionsort example_list = result);;
