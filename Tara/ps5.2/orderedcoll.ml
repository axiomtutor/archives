...
module BinSTree (Elt : COMPARABLE)
  ...
  struct
    ...
    let rec insert (x : elt) (t : tree) : tree =
      ...
      | Branch (l1, hd :: tl, l2) -> (match Elt.compare x hd with 
                                      | Less -> Branch (insert x l1, hd :: tl, l2)
                                      | Equal -> Branch (l1, x :: hd :: tl, l2)
                                      | Greater -> Branch (l1, hd :: tl, insert x l2))
      | _ -> failwith "this line shouldn't be reachable" ;;
    ...
    let rec pull_max (t : tree) : elt list * tree =
      ...
      | Branch (left, this, right) -> let max, right' = pull_max right in
                                      (max, Branch (left, this, right'))
    
    let rec getmax (t : tree) : elt =
      let (l, _) = pull_max t in 
      match l with 
      | hd :: _ -> hd 
      | _ -> failwith "this line shouldn't be reachable" ;;
