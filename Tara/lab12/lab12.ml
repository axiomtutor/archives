...
let incr (reg : int ref) : int ref = 
  reg.contents <- reg.contents+1;
  reg;;
...
print_endline (remember "ex");
print_endline (remember "ex2");;
...
let rec mlist_of_list (lst : 'a list) : 'a mlist =
  match lst with 
  | [] -> Nil
  | h::tl -> Cons (h, ref (mlist_of_list tl));;
